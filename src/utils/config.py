...
class DataConfig:
    ...
    # ── NIH ChestX-ray14 ─────────────────────────────────────────
    # 12 zip files, each ~2GB. The original nihcc.box.com static links
    # expire periodically; these are mirrored on HuggingFace with
    # original filenames intact (see nih_subset_url below).
    nih_download_urls: list[str] = [
        f"https://huggingface.co/datasets/alkzar90/NIH-Chest-X-ray-dataset"
        f"/resolve/main/data/images/images_{i:03d}.zip"
        for i in range(1, 13)
    ]
